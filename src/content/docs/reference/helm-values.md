...
| `namespace` | `agent-engine-system` | Install namespace; must match the base manifests for no-drift. |
...
| `bff.mcp.credentialNamespace` | `""` | Set (e.g. `agent-engine-credentials`) to render a locked platform namespace holding MCP grant Secrets so tenants can't read each other's OAuth tokens. **Production should set this.** |
